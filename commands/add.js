...
module.exports = {
	...
	description: 'Thêm tài khoản vào service.', // Command description displays in the help command
	...
	execute(message, args) {
        ...
        if (!service) {
            return message.channel.send(
                ...
                .setTitle('Thiếu thông số!')
                .setDescription('Bạn cần chỉ định một dịch vụ!')
                .addField('Ví dụ', `${config.prefix}${this.name} **tree** apple`)
        ...
        if (!account) {
            return message.channel.send(
                ...
                .setColor(config.color.red)
                .setTitle('Thiếu thông số!')
                .setDescription('Bạn cần chỉ định một tài khoản!')
                .addField('Ví Dụ', `${config.prefix}${this.name} tree **apple**`)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );            
        ...
        fs.appendFile(filePath, `${os.EOL}${args[1]}`, function (error) {
            ...
            message.channel.send(
                ...
                .setTitle('Đã thêm tài khoản!')
                .setDescription(`Thêm thành công \`${args[1]}\` tài khoản để \`${args[0]}\` service!`)
